...
public class ToggleKeyHandler {
    private static final KeyMapping TOGGLE_KEY = new KeyMapping("squake.key.toggle", GLFW.GLFW_KEY_COMMA, KeyMapping.Category.register(ResourceLocation.parse("squake")));
    ...
    private static void onKeyEvent(InputEvent.Key event)
    {
        if(TOGGLE_KEY.consumeClick())
        {
            ...
            var onOrOff = MutableComponent.create(new TranslatableContents(SquakeConfig.isEnabled() ? "squake.key.toggle.enabled" : "squake.key.toggle.disabled", null, new Object[0])).withStyle(SquakeConfig.isEnabled() ? ChatFormatting.GREEN : ChatFormatting.DARK_RED);
