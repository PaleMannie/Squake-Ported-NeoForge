...
public class SquakeClientPlayer {
    ...
    public static boolean quake_moveEntityWithHeading(Player player, float sidemove, float upmove, float forwardmove)
    {
        ...
        {
            if(SquakeConfig.sharkingEnabled)
        ...
        } else
        {
            ...
            else
            {
                ...
                if(SquakeConfig.sharkingEnabled && SquakeConfig.sharkingSurfTension > 0.0D && isJumping(player) && Motions.getMotionY(player) < 0.0F)
                {
                    ...
                    if(isFallingIntoWater)
                        Motions.setMotionY(player, Motions.getMotionY(player) * SquakeConfig.sharkingSurfTension);
                }
            }
    ...
    {
        if(SquakeConfig.trimpingEnabled && player.isShiftKeyDown())
        {
            ...
            if(curspeed > movespeed)
            {
                ...
                Motions.setMotionY(player, Motions.getMotionY(player) + speedbonus * curspeed * SquakeConfig.trimpMult);

                if(SquakeConfig.trimpMult > 0)
                {
                    float mult = (float) (1.0f / SquakeConfig.trimpMult);
                    double motionX = Motions.getMotionX(player), motionZ = Motions.getMotionZ(player);
                    motionX *= mult;
                    motionZ *= mult;
                    Motions.setMotionHoriz(player, motionX, motionZ);
    ...
    private static void quake_WaterMove(Player player, float sidemove, float upmove, float forwardmove)
    {
        ...
        {
            if(curspeed > 0.09)
                quake_ApplyWaterFriction(player, SquakeConfig.sharkingWaterFriction);

            if(curspeed > 0.098)
                quake_AirAccelerate(player, wishspeed, wishdir[0], wishdir[1], SquakeConfig.accelerate);
            else
                quake_Accelerate(player, .0980F, wishdir[0], wishdir[1], SquakeConfig.accelerate);
